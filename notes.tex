...
\\ i.d.d property: \(P(X_1 = a_1, \ldots, X_n = a_n) = \prod_{i=1}^n P(X_i=a_i)\)
   ...
   \\
    \\(Note: \({a,b,c}^n\) denotes the cartesian product of the set \(a,b,c\) n times)
   \\ K denotes the number of bits in each word rounded UP to the nearest integer
...
\[\frac{1}{n} \sum^n_{i=1} x_i \xrightarrow{n\to\infty} E[X]\] in probability
    ...
    \[R\ achieveable \Longleftrightarrow \forall \epsilon>0, \exists \text{ D-ary (k,n) fixed length codes } \rho_n \text{ such that } \limsup_{n \to \infty}\frac{k}{n}\leq R\]
